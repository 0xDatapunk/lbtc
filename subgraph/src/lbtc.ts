    ...
    if (lastTimestamp != BigInt.fromI32(0)) {
      ...
      if (lastSnapshot) {
        ...
        if (lastSnapshot.point) {
          let pointStr = lastSnapshot.point!.toString()
          // Use BigInt to parse and convert to number safely
          let intValue = BigInt.fromString(pointStr).toI32()
          lastPoint = intValue
        }
...
function createAndSaveSnapshot(
  ...
  let account = Accounts.load(accountId) as Accounts
